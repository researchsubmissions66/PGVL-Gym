"""Strict prompt loading for FOCUS's native dual-scale text banks."""
...
FOCUS_PROMPT_FORMAT = POSITIONAL_PROMPT_FORMAT
FOCUSPromptBank = PositionalPromptBank


def focus_prompt_bank_sha256(
    class_names: Sequence[str],
    low_resolution: Sequence[str],
    high_resolution: Sequence[str],
) -> str:
    return positional_prompt_bank_sha256(
        class_names, low_resolution, high_resolution, consumer="FOCUS")


def load_focus_prompt_bank(
    path: str | Path,
    *,
    class_names: Sequence[str],
    ...
    """Load and bind a native headerless FOCUS prompt CSV."""
