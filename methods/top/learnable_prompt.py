    ...
    def __init__(self, n_ctx=0, ctx_init="", all_ctx_trainable=True, csc=True, classnames=["Negative", "Positive"], clip_model='RN50', p_drop_out=0.0):
        ...
        if ctx_init:
            if not self.use_class_specific_ctx:
                ...
                prompt = clip.tokenize(ctx_init).to(embedding_device)
                with torch.no_grad():
                ...
                for ctx_init_i in ctx_init:
                    ...
                    prompt_i = clip.tokenize(ctx_init_i).to(embedding_device)
                    with torch.no_grad():
        ...
        tokenized_prompts = torch.cat(
            [clip.tokenize(p) for p in prompts]).to(embedding_device)
        with torch.no_grad():
...
class MIL_CLIP(nn.Module):
    ...
    def forward(self, image_raw):
        ...
        # Precomputed bags are normally float32 while OpenAI CLIP text
        # features are float16 on CUDA. TOP's trainable pooling/attention
        # layers are float32, so perform the cross-modal comparisons in that
