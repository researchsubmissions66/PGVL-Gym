...
class MInterface(pl.LightningModule):
    def __init__(self, gc, seed, num_shots, max_epochs, model_name, n_vpro, wandb_name, txt_result_path, heatmap_path, dataset_name, num_k, **kargs):
        super().__init__()
        ...
        self.num_k = num_k #TODO 消融实验，到时候删除
    ...
    def validation_epoch_end(self, outputs):
        ...
        self.log('val_f1', f1, on_step=False, on_epoch=True, prog_bar=True, batch_size=1)
        self.log('val_acc', acc, on_step=False, on_epoch=True, prog_bar=True, batch_size=1)
        if self.dataset_name in ['RCC', 'UBC-OCEAN']:
        ...
        if epoch_score > self.best_val_score:
            ...
            if self.num_k is not None:
                with open(os.path.join(self.txt_result_path, self.wandb_name+'_numK_'+str(self.num_k)+'.json'), 'w', encoding='utf-8') as f:
