...
class MYPromptLearner(nn.Module):
    ...
    def forward(self, feats, desc):
        ...
        tokenized_prompts = torch.from_numpy(np.array(tokenized_prompts)).to(self.p_input.device)
        attention_mask = torch.from_numpy(np.array(attention_mask)).to(self.p_input.device)
...
class CustomCLIP(nn.Module):
    def __init__(self, classnames, clip_model, gpt_dir, dataset_name,
        ...
        # tokenize
        tokenizer = tokenizer or load_pretrained_tokenizer(base_model)
    ...
    def forward(self, big_image, small_embeddings, train=True):
        ...
        if small_embeddings.shape[-1] == projection.out_features:
            # Benchmark bags may already be in the paired 512-D shared space.
            image_features_zs = F.normalize(small_embeddings, dim=1)
        elif small_embeddings.shape[-1] == projection.in_features:
            image_features_zs = F.normalize(
                projection(small_embeddings), dim=1)
        else:
            raise ValueError(
                "MSCPT low features must be pre-projection width "
                f"{projection.in_features} or shared width "
                f"{projection.out_features}, got {small_embeddings.shape[-1]}")
        ...
        if big_image.ndim == 2 and big_image.shape[-1] == projection.out_features:
            # Feature-only benchmark mode: the high-resolution bag has
            # already passed through the paired visual tower.
