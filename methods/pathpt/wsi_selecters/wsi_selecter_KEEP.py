...
def wsi_prompt_selector(train_wsi_loader, prompt_lst, PPL_model = None, device = 'cuda:0'):
    ...
    # Get indices of top select_num maximum values (if list length is insufficient, get all)
...
def patch_prompt_selector(train_dataset, dataset_name, param, device='cuda:0', k=10):
    ...
    param (dict): Parameter dictionary containing train_dir_path and zero_shot_template_file etc.
    device (str): Device to use, default 'cuda:0'
    k (int): Number of prompts to select for each class, default 10
