...
def generate_patch_label_3d(logits, logits_thd, wsi_labels):
    ...
    Input:
        ...
        wsi_labels: [batch_size] original labels for each sample (0-based)
    Output:
...
def wsi_prompt_selector(base_model, train_wsi_loader, prompt_lst, PPL_model = None, device = 'cuda:0'):
    ...
    # Get indices of the top select_num largest values (if list length is less than select_num, get all)
...
def patch_prompt_selector(train_dataset, dataset_name, param, device='cuda:0', k=10):
    """
    Select the best prompts for each class
    
    Parameters:
    dataset (str): Dataset name, e.g. 'BRAIN'
    ...
    # Return selected prompts as a dictionary
