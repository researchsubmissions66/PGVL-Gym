...
def generate_patch_label_3d(logits, logits_thd, wsi_labels):
    ...
    # Get max logit and corresponding class for each patch
    max_values, max_indices = torch.max(logits, dim=2)  # [batch_size, num_patches]

    # Initialize all patch labels as -wsi_label (using broadcast)
...
def wsi_prompt_selector(musk_model, musk_tokenizer, train_wsi_loader, prompt_lst, PPL_model = None, device = 'cuda:0'):
    ...
    # Get indices of top select_num largest values (if list length is less than select_num, get all)
...
#     param (dict): Parameter dictionary containing train_dir_path, zero_shot_template_file, etc.
